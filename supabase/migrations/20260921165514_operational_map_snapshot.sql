-- Map geometries stay private. This RPC exposes only a role-filtered, flat marker
-- projection suitable for the browser map, never raw tables to anonymous users.
...
begin
  ...
  select
    ...
    st_y(work_order.location),
    st_x(work_order.location),
    work_order.status::text,
    work_order.updated_at
  from public.work_orders work_order
  where work_order.scheduled_for = p_day
    and work_order.location is not null;

  return query
  with latest_locations as (
    select distinct on (location.team_id)
      location.team_id,
      location.location,
  ...
  select
    ...
    team.id,
    team.code,
    concat('Cuadrilla ', team.code),
    st_y(latest.location),
    st_x(latest.location),
    coalesce(current_order.status::text, 'available'),
    latest.recorded_at
  ...
  left join lateral (
    ...
    from public.work_orders work_order
    where work_order.assigned_team_id = team.id
      and work_order.scheduled_for = p_day
...
revoke all on function public.operational_map_snapshot(date) from public;
