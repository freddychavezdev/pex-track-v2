...
export class OperationalMapComponent implements AfterViewInit, OnChanges, OnDestroy {
  ...
  renderMarkers(): void {
    ...
    validMarkers.forEach((marker) => {
      ...
      const signalClass = marker.marker_type === 'team' ? (this.hasLiveSignal(marker) ? ' signal-online' : ' signal-offline') : '';
      ...
      const markerContent = marker.marker_type === 'team'
        ? `<span class="operational-marker team${signalClass}${deviationKm !== null ? ' deviation' : ''}"><span class="marker-symbol">${crewVehicle}</span>${deviationKm !== null ? '<b class="deviation-badge" aria-label="Posible desvío">!</b>' : ''}</span>`
  ...
  private hasLiveSignal(marker: OperationalMapMarker): boolean {
    return marker.status !== 'suspended' && Boolean(marker.observed_at)
      && Date.now() - new Date(marker.observed_at).getTime() <= 90 * 1000;
  }
